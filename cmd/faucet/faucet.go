...
import (
	...
	"github.com/relianz2019/relianz/ethclient"
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Relianz protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Relianz and the faucet")
)

var (
	rlz = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // Relianz address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Relianz light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Relianz protocol stack
	client *ethclient.Client   // Client connection to the Relianz chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Relianz light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := rlz.DefaultConfig
...
// close terminates the Relianz connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the Relianz address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/relianz2019/relianz/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), rlz)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance = new(big.Int).Div(balance, rlz)
...
// the username, avatar URL and Relianz address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for Relianz addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Relianz address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Relianz address and profile URL.
...
// returning the username, avatar URL and Relianz address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Relianz address,
