...
import (
	...
	"github.com/relianz2019/relianz/internal/jsre"
	"github.com/relianz2019/relianz/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute Relianz requests through
...
func (c *Console) init(preload []string) error {
	...
	flatten := "var rlz = web3.rlz; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = rlz.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines rlz.getBalance(rlz.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
func (c *Console) Welcome() {
	...
	c.jsre.Run(`
		...
		console.log("coinbase: " + rlz.coinbase);
		console.log("at block: " + rlz.blockNumber + " (" + new Date(1000 * rlz.getBlock(rlz.blockNumber).timestamp) + ")");
