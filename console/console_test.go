...
func newTester(t *testing.T, confOverride func(*rlz.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Relianz service within
	...
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return rlz.New(ctx, ethConf) }); err != nil {
		t.Fatalf("failed to register Relianz protocol: %v", err)
	}
