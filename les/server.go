...
func NewLesServer(rlz *rlz.Relianz, config *rlz.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(rlz.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, rlz.EventMux(), rlz.Engine(), newPeerSet(), rlz.BlockChain(), rlz.TxPool(), rlz.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(rlz.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(rlz.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(rlz.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(rlz.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(rlz.ChainDb())
	return srv, nil
}
