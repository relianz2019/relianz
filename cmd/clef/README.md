Clef
----
Clef can be used to sign transactions and data and is meant as a replacement for rlz's account management.
This allows DApps not to depend on rlz's account management. When a DApp wants to sign data it can send the data to
the signer, the signer will then provide the user with context and asks the user for permission to sign the data. If
the users grants the signing request the signer will send the signature back to the DApp.
  
This setup allows a DApp to connect to a remote Relianz node and send transactions that are locally signed. This can
help in situations when a DApp is connected to a remote node because a local Relianz node is not available, not
synchronised with the chain or a particular Relianz node that has no built-in (or limited) account management.
...
The general flow for signing a transaction using e.g. rlz is as follows:
![image](sign_flow.png)

In this case, `rlz` would be started with `--externalsigner=http://localhost:8550` and would relay requests to `rlz.sendTransaction`.
...
put together is a bit of a hack into the http server. This could probably be greatly improved
    - Relay: Rlz should be started in `rlz --external_signer localhost:8550`.
...
* Another potential thing to introduce is pairing.
  ...
  * Thus rlz/mist/cpp would cryptographically handshake and afterwards the caller would be allowed to make signing requests.
...
The signer will generate a new private key, encrypts it according to [web3 keystore spec](https://github.com/relianz/wiki/wiki/Web3-Secret-Storage-Definition) and stores it in the keystore directory.
...
#### Arguments
  - account [object]: key in [web3 keystore format](https://github.com/relianz/wiki/wiki/Web3-Secret-Storage-Definition) (retrieved with account_export) 
...
#### Result
  - exported key, see [web3 keystore format](https://github.com/relianz/wiki/wiki/Web3-Secret-Storage-Definition) for
  more information
  ...
  "params": [
    {
      "address": "0x123409812340981234098123409812deadbeef42",
      "raw_data": "0x01020304",
      "message": "\u0019Relianz Signed Message:\n4\u0001\u0002\u0003\u0004",
...
* A Graphical UI MUST show the blocky-identicon for relianz addresses.
