...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of rlz is "rlz". If no
...
// These resources are resolved differently for "rlz" instances.
...
func (c *Config) resolvePath(path string) string {
	...
	// by rlz 1.4 are used if they exist.
	if c.name() == "rlz" && isOldGethResource[path] {
		oldpath := ""
		if c.Name == "rlz" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-relianz-keystore")
		ephemeral = keydir
	}
