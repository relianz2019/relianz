...
// Service implements an Relianz netstats reporting daemon that pushes local
// chain statistics up to a monitoring server.
type Service struct {
	server *p2p.Server        // Peer-to-peer server to retrieve networking infos
	rlz    *rlz.Relianz      // Full Relianz service if monitoring a full node
	les    *les.LightRelianz // Light Relianz service if monitoring a light node
...
func New(url string, ethServ *rlz.Relianz, lesServ *les.LightRelianz) (*Service, error) {
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["rlz"]; info != nil {
		network = fmt.Sprintf("%d", info.(*rlz.NodeInfo).Network)
		protocol = fmt.Sprintf("rlz/%d", rlz.ProtocolVersions[0])
	} else {
...
func (s *Service) assembleBlockStats(block *types.Block) *blockStats {
	...
	if s.rlz != nil {
		...
		if block == nil {
			block = s.rlz.BlockChain().CurrentBlock()
		}
		header = block.Header()
		td = s.rlz.BlockChain().GetTd(header.Hash(), header.Number.Uint64())
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(list) > 0 {
		...
		var head int64
		if s.rlz != nil {
			head = s.rlz.BlockChain().CurrentHeader().Number.Int64()
		} else {
	...
	for i, number := range indexes {
		...
		if s.rlz != nil {
			block = s.rlz.BlockChain().GetBlockByNumber(number)
		} else {
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	if s.rlz != nil {
		mining = s.rlz.Miner().Mining()
		hashrate = int(s.rlz.Miner().HashRate())

		sync := s.rlz.Downloader().Progress()
		syncing = s.rlz.BlockChain().CurrentHeader().Number.Uint64() >= sync.HighestBlock

		price, _ := s.rlz.APIBackend.SuggestPrice(context.Background())
