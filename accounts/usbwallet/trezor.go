...
import (
	...
	"github.com/relianz2019/relianz/accounts/usbwallet/internal/trezor"
...
// or whether the Relianz app was not started on it.
...
// Relianz address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.RelianzAddress)
	if _, err := w.trezorExchange(&trezor.RelianzGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.RelianzTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.RelianzTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
