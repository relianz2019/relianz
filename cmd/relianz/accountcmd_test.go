...
func TestAccountListEmpty(t *testing.T) {
	rlz := runGeth(t, "account", "list")
	rlz.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	rlz := runGeth(t, "account", "list", "--datadir", datadir)
	defer rlz.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	rlz := runGeth(t, "account", "new", "--lightkdf")
	defer rlz.ExpectExit()
	rlz.Expect(`
...
`)
	rlz.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	rlz := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer rlz.ExpectExit()
	rlz.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(rlz.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(rlz.StderrText(), m) {
...
// https://github.com/relianz2019/relianz/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	rlz.SetTemplateFunc("keypath", func(file string) string {
