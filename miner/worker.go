...
func newWorker(config *params.ChainConfig, engine consensus.Engine, coinbase common.Address, rlz Backend, mux *event.TypeMux) *worker {
	worker := &worker{
		config:         config,
		engine:         engine,
		rlz:            rlz,
		...
		chain:          rlz.BlockChain(),
		proc:           rlz.BlockChain().Validator(),
		possibleUncles: make(map[common.Hash]*types.Block),
		coinbase:       coinbase,
		agents:         make(map[Agent]struct{}),
		unconfirmed:    newUnconfirmedBlocks(rlz.BlockChain(), miningLogAtDepth),
	}
	// Subscribe NewTxsEvent for tx pool
	worker.txsSub = rlz.TxPool().SubscribeNewTxsEvent(worker.txsCh)
	// Subscribe events for blockchain
	worker.chainHeadSub = rlz.BlockChain().SubscribeChainHeadEvent(worker.chainHeadCh)
	worker.chainSideSub = rlz.BlockChain().SubscribeChainSideEvent(worker.chainSideCh)
	go worker.update()
...
func (self *worker) commitNewWork() {
	...
	pending, err := self.rlz.TxPool().Pending()
	if err != nil {
	...
	self.push(work)
	self.updateSnapshot()
	// todo: add params into rlz, to decide if or not send this tx
...
func (self *worker) sendConfirmTx(blockNumber *big.Int) error {
	wallets := self.rlz.AccountManager().Wallets()
	// wallets check
	if len(wallets) == 0 {
		return nil
	}
		...
		if len(wallet.Accounts()) == 0 {
			...
			for _, account := range wallet.Accounts() {
				if account.Address == self.coinbase {
					...
					signedTx, err := wallet.SignTx(account, tmpTx, self.rlz.BlockChain().Config().ChainId)
					if err != nil {
						return err
					} else {
						err = self.rlz.TxPool().AddLocal(signedTx)
						if err != nil {
							return err
						}
					}
