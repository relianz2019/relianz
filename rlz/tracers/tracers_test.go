...
// To generate a new callTracer test, copy paste the makeTest mrlzod below into
// a Grlz console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = rlz.getBlock(rlz.getTransaction(tx).blockHash);
  var genesis = rlz.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.rlz.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  rlz.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
...
func TestCallTracer(t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(strings.TrimPrefix(file.Name(), "call_tracer_"), ".json")), func(t *testing.T) {
			...
			statedb := tests.MakePreState(rlzdb.NewMemDatabase(), test.Genesis.Alloc)
